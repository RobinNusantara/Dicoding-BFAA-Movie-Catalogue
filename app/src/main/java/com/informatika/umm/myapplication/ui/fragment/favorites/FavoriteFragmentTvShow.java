...
public class FavoriteFragmentTvShow extends Fragment implements ItemClickListener {
    ...
    private void initFavoriteTask() {
        favoriteTask = new GetTvShowFavoriteTask(this);
    }
    ...
    private void setupRefreshLayout() {
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                refreshFavoriteTvShow();
            }
        });
    }

    private void refreshFavoriteTvShow() {
        favoriteTask = null;
        initFavoriteTask();
        favoriteTask.execute();
    }
    ...
    private static class GetTvShowFavoriteTask extends AsyncTask<Void, Void, List<Movie>> {
        ...
        private WeakReference<FavoriteFragmentTvShow> fragment;

        GetTvShowFavoriteTask(FavoriteFragmentTvShow fragment) {
            this.fragment = new WeakReference<>(fragment);
        }

        @Override
        protected List<Movie> doInBackground(Void... voids) {
            String KEY = Objects.requireNonNull(fragment.get().getContext()).getString(R.string.str_key_shows);
            return RoomClient
